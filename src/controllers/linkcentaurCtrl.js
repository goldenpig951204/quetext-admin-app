...
const login = async (req, res) => {
    ...
    try {
        ...
        if (response.request.path == "/") {
            ...
            log("linkcentaur").info(`Start session with ${email} successfully.`);
            res.send("Login successfully.");
        } else {
            res.status(500).send("Credential is incorrect.");
        }
    } catch (err) {
        log("linkcentaur").error(`Start session with ${email} failed: ${get(err, "response.data.message") || err.toString()}`);
