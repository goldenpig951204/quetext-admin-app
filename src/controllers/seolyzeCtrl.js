...
const login = async (req, res) => {
    ...
    try {
        ...
        if (typeof data.action !== "undefined") {
            ...
            log("seolyze").info(`Start session with ${username} successfully.`);
            res.send("Login successfully.");
        } else {
            res.status(500).send("Credential is incorrect.");
        }
    } catch (err) {
        log("seolyze").error(`Start session with ${username} failed: ${get(err, "response.data.message") || err.toString()}`);
